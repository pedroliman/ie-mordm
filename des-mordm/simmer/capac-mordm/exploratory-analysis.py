...
model = RModel("modelo.R", "modelo_capacitores", RCMD=r"/usr/lib/R/bin/R")
    
# The parameter names must match the R arguments exactly
model.parameters = [Parameter("v_capacidades"),
                    Parameter("v_custos"),
                    Parameter("v_tempos_ciclo"),
                    Parameter("reps"),
                    Parameter("tempo_sim")]
...
model.responses = [Response("producao", Response.MAXIMIZE),
                   Response("custo", Response.MINIMIZE)]

# Specify the levers
model.levers = [IntegerLever("v_capacidades", 1, 10, length=4)]
